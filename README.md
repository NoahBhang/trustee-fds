...
현재 기본 CLI는 값이 있으면 `suspension_date_confirmed=N`이어도 후보 누락을 줄이기
위한 잠정 기준점으로 시간 창과 `post_anchor` 신호에 포함한다. 실행 시 이 정책과
해당 사건을 경고로 먼저 표시한다. 따라서 그 신호는 확정 사실이 아니며, 아래의
미특정·대체일 시나리오와 함께 읽어야 한다. 시나리오 자동 실행·비교는 로드맵 6이다.
...
다만 이 파일은 독립 기관이 만든 정답 데이터가 아니라, 현재 엔진 출력에서 출발해
수동 검토한 **회귀 계약**이다. 정확 대조는 구현 변화와 예기치 않은 출력 변화를 잘
잡지만 법률적 타당성을 독립적으로 증명하지 않는다. 신호·플래그 순서도 현재는 출력
계약에 포함되므로 정당한 순서 변경에도 기대값 갱신이 필요하다.
...
이때 T027의 `Value Out=30,000,000`, `Value In=0`은 분석 단위를 초과변제 부분으로
잘라 표시한 값이다. 실제 채무 감소 100,000,000원이 없었다는 뜻이 아니다. CLI는
실제 지급액·채무 감소액·초과분을 별도 산식으로 출력해 이 정규화가 사실관계 누락으로
오해되지 않게 한다.
...
`transaction_links` 참조·시간순·자기참조·모든 다중 노드 순환 구성요소.
...
**exit code(`src/cli.py`)는 상호 배타적인 상태 코드다.** CRITICAL 입력은 판정과
`validate()` 전에 즉시 2로 끝나므로, 1과 2가 결합된 값은 나오지 않는다.

| 코드 | 의미 |
|---:|---|
| `0` | 입력 무결성·골든 대조 모두 통과 |
| `1` | 골든파일 불일치 또는 `target_stage` 미도달 |
| `2` | CRITICAL 입력 무결성 문제 — 골든 대조 미실행 |

CRITICAL이 있어도 0을 반환하던 fail-open 문제는 해결하되, 먼저 입력을 고친 뒤
회귀 대조를 수행하도록 검사 순서를 명확히 분리했다.
...
- 상대방 복수 시 출연 가치의 **지분별 배분**. `share_ratio` 합의 정합성은
